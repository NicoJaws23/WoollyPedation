...
This model will simulate woolly monkey predation responses. The woollys will have a certai distance from which they have a CHANCE to detect the predator and then remeber the location where they encountered said predator. Additioanlly, the woollys will be in a group of a certain number of individuals which will periodically split into a number of different groups dependning on resource availibility. These groups will be able to communicate with each other and in the event a predator is detected, share information about where the enounter took place. The woollys will also leave evidence of their presence which predators can detect.

Predators will also have a detection distance with a higher chance of being able to detect the woollys. They will be able to use evidence of woolly monkey presence (feces) and will also leave evidence of their own presence which the woollys can pick up on and subsequently avoid. 

Woolly monkeys can reproduce with each other and so can predators but to a lesser degree. Eventually, this model will incorportate a height element, where there are both terrestrial and aerial predators which the woollies must avoid.Additioanlly, predators will eventually be limited to a single location which will act as a landscape of fear.

## 2. Entities, State Variables, and Scales

Entities include woolly monkeys, predators, and fruiting resources

State variables include vegetation, energy, requirments to reproduce
